...
│ ────────────────────────────────────────────────────────── │
│ 核心能力：设计可复用、可组合的 AI 系统                       │
│ 标志：能设计 MCP 服务器、多代理编排系统                     │
│ 时间视野：1+ 月，影响整个团队                               │
└─────────────────────────────────────────────────────────────┘
         ↑ 能设计新系统
┌─────────────────────────────────────────────────────────────┐
...
│ ────────────────────────────────────────────────────────── │
│ 核心能力：将重复性工作转化为自动化流程                       │
│ 标志：能创建 Slash Commands、Warp 自动化                    │
│ 时间视野：1-4 周，影响个人效率                              │
└─────────────────────────────────────────────────────────────┘
         ↑ 能识别瓶颈并设计自动化
┌─────────────────────────────────────────────────────────────┐
...
L2 做法（协作工程师）：
  "我需要设计 Notes 前端，应该用什么技术栈？"
  → AI 给建议：React + TypeScript
  → 我理解了权衡：为什么用 React 而不是 Vue
  → 学到了：前端架构决策
  
L3 做法（流程自动化者）：
  观察："我每周都要为新 API 创建前端"
  → 设计：创建可复用的前端生成模板
  → 实现：Slash Command + 配置文件
  → 学到了：如何抽象重复模式
  
L4 做法（系统架构师）：
  思考："如何设计一个前端生成系统？"
  → 设计：API schema → 前端组件的映射系统
  → 实现：多代理协作（schema parser + code generator + reviewer）
  → 学到了：系统设计和代理编排
```
...
**定义**：知道使用哪些 AI 工具，能完成基本任务

**能力标志**：
- ✅ 能使用 Claude Code、ChatGPT、Copilot
- ✅ 能调用 LLM API
- ✅ 能让 AI 生成简单代码
...
**晋级条件 (L1 → L2)**：
- [ ] 能独立设计 4-Layer Prompt（Persona → Process → Template → Validation）
- [ ] 能预测 AI 在哪些情况下会失败
- [ ] AI 生成的代码，修改率 <10%
- [ ] 理解何时用 AI、何时手工实现
...
放大效应: 无（基础层，不放大其他能力）
被放大效应:
  ← 被 L2 Prompt Engineering 放大（更好的工具选择）
...
**定义**：能有效地与 AI 协作开发，理解 AI 的边界

**能力标志**：
- ✅ 能写出高质量、结构化的 Prompt
- ✅ 能在 AI 辅助下完成完整功能
- ✅ 能评估 AI 输出的质量并快速修正
- ✅ 理解 AI 的优势（快速原型）和劣势（复杂逻辑）

**典型任务**：
- 用 AI 辅助完成 Week 2 的 Notes API
- 设计 Prompt 让 AI 生成测试用例
- 用 AI Code Review 发现安全问题

**晋级条件 (L2 → L3)**：
- [ ] 能识别重复性任务并量化成本（"这个任务我每周做 3 次，每次 20 分钟"）
- [ ] 创建了至少 1 个可复用的自动化（Slash Command/Warp Rule）
- [ ] 能解释自动化的输入/输出契约
- [ ] 考虑了失败处理和回滚机制
...
**系统影响** (Meadows):
```
核心能力: Prompt Engineering
放大效应:
  → L3 Automation: 更好的 prompt → 更好的自动化
  → L4 System Design: 更好的需求描述 → 更好的系统设计
被放大效应:
  ← L1 Tool Knowledge: 更懂工具 → 更好的 prompt
  ← L3 Automation: 自动化实践 → 知道什么 prompt 有效
网络效应: Prompt 质量影响所有 AI 协作效率
```

**Job-to-be-Done: 何时练习 L2？** (Christensen):
```
核心问题: "与 AI 有效协作完成复杂任务"
...
放大效应:
  → L2 Prompt Engineering: 自动化模式 → 更好的 prompt 模板
  → L4 System Design: 自动化实践 → 系统设计经验
  → 每个自动化让我的 prompt 更清晰（循环增强）

被放大效应:
  ← L2 Prompt Engineering: 能描述复杂自动化
...
**典型任务**：
- 设计类似 SuperClaude 的系统
- 创建新的 MCP Protocol 实现
- 设计企业级 AI 工作流平台
...
放大效应:
  ...
  → L2 Prompt Engineering: 系统思维 → 更好的 prompt 结构
  → 所有其他能力: 系统设计放大所有能力的价值

被放大效应:
  ← L3 Automation: 自动化实践积累的系统直觉
  ← L2 Prompt Engineering: 清晰的需求描述
...
**如果选 B，这个任务需要什么层级的能力？**
- [ ] L1: 工具使用（调用 API、用工具生成）
- [ ] L2: 协作工程（设计 Prompt、理解边界）
- [ ] L3: 流程自动化（设计可复用模式）⭐
- [ ] L4: 系统架构（设计新的系统）
...
|------|---------|------------|---------|
| **巩固 L1** | 快速使用工具 | AI 作为执行者 | 任务完成 |
| **提升到 L2** | 先设计 Prompt 再执行 | AI 作为评审者 | Prompt 质量高 |
| **提升到 L3** | 强制抽象为可复用模式 | AI 作为苏格拉底导师 | 设计了可复用系统 |
| **提升到 L4** | 设计多组件协作系统 | AI 作为架构评审 | 系统可被他人用 |
...
### 2. 系统网络效应分析 ⭐
**网络连接**:
- 连接了：L2 Prompt Engineering（需要 prompt 描述代理）
- 新创建了：无（没有形成可复用模式）

**放大效应**:
- 这个任务没有明显放大其他能力
- 被 L2 放大：我的 prompt 能力让代理能运行
